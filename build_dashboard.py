...
tabs='<button class="tab" data-d="packing" onclick="show(\'packing\')">Packing Summary</button>'
...
pack_days=[
 {"t":"Volcano tour — La Fortuna","d":"Thu Jul 23","img":I["springs"],
  "items":[("Swimwear (waterfall + hot springs)",True),("Quick-dry towel",True),
   ("Sandals / flip-flops for the springs",True),("Dry change of clothes for after the springs",True),
   ("Dry bag or waterproof phone pouch",True),("Light rain layer (mountain weather)",True),
   ("Comfortable closed-toe shoes for the lava-trail hike",True),
   ("Sunscreen",False),("Sunglasses",False),("Bug spray",False),("Water + cash for drinks/tips",False)]},
 {"t":"ATV jungle tour","d":"Fri Jul 24","img":I["atv"],
  "items":[("Closed-toe shoes you can trash (gets muddy)",True),("Clothes you're fine throwing away",True),
   ("Bandana or buff for dust",True),("Long socks",True),
   ("Sunscreen",False),("Sunglasses",False),("Bug spray",False),("Water + cash for drinks/tips",False)]},
]
def packlist(items):
    return '<ul class="pklist">'+"".join(f'<li>{("<strong>"+t+"</strong>") if b else t}</li>' for t,b in items)+'</ul>'
pcards=""
for p in pack_days:
    pcards+=f'''<article class="card"><div class="photo"><img src="{p['img']}" loading="lazy"></div><div class="cb"><div class="ct"><span class="tg">Pack for</span><span class="tm">{p['d']}</span></div><h3>{p['t']}</h3>{packlist(p['items'])}</div></article>'''
panels=(f'''<section class="panel" id="packing"><header class="hero"><img src="{I['falls']}"><div class="sc"></div><div class="ht"><div class="brand">Manish &amp; Sags De<span class="bx">BACH</span>ery</div><div class="kick">Packing Summary</div><h1>What to pack for the tour days</h1><div class="chips"><span class="chip">Volcano tour · Thu 23</span><span class="chip">ATV tour · Fri 24</span><span class="chip">Bold = day-specific</span></div></div></header><main class="wrap"><div class="pknote">Both tour days share the basics. <strong>Bold items are specific to that day</strong>; plain items you'll want on both. Everything fits in a small drawstring bag.</div>{pcards}<div class="foot">Manish &amp; Sags DeBACHery · Costa Rica · Jul 22-26 · tap any spot for Maps</div></main></section>''')+panels
